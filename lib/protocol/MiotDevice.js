...
class MiotDevice extends EventEmitter {
  ...
  async _connectToCloudDevice() {
    if (!this.isConnectedToCloud()) {
      if (this._canUseMiCloud()) {
        ...
        try {
          this.logger.info(`Connecting to MiCloud!`);
          
          // Check if we have tokens instead of username/password
          const serviceToken = this._getMiCloudServiceToken();
          const userId = this._getMiCloudUserId();
          const ssecurity = this._getMiCloudSSecurity();
          
          if (serviceToken && userId && ssecurity) {
            // Use token-based authentication
            this.logger.debug(`Using token-based MiCloud authentication`);
            this.miCloud.setServiceToken({
              serviceToken: serviceToken,
              userId: userId,
              ssecurity: ssecurity
            });
          } else {
            // Fallback to traditional login (for backward compatibility)
            const username = this._getMiCloudUsername();
            const password = this._getMiCloudPassword();
            if (username && password) {
              this.logger.debug(`Using username/password MiCloud authentication`);
              await this.miCloud.login(username, password);
            } else {
              throw new Error('Neither tokens nor username/password provided for MiCloud authentication');
            }
          }
  ...
  _canUseMiCloud() {
    // Check for new token-based authentication first
    const hasTokens = this._getMiCloudServiceToken() && this._getMiCloudUserId() && this._getMiCloudSSecurity();
    // Fallback to old username/password authentication
    const hasCredentials = this._getMiCloudUsername() && this._getMiCloudPassword();
    
    return this.miCloudConfig && (hasTokens || hasCredentials);
  }
  ...
  _getMiCloudServiceToken() {
    return this._getMiCloudConfigProp('serviceToken');
  }

  _getMiCloudUserId() {
    return this._getMiCloudConfigProp('userId');
  }

  _getMiCloudSSecurity() {
    return this._getMiCloudConfigProp('ssecurity');
  }
